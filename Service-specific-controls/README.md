...
|[Deny users from using long term Amazon Bedrock API keys](Amazon-Bedrock/Deny-Bedrock-long-term-api-keys.json) | Restrict users and roles from calling Amazon Bedrock API and Amazon Bedrock Powered by AWS Mantle endpoints with long-term bearer tokens while allowing short-term bearer tokens. This provides a middle ground by permitting temporary credentials while blocking persistent API keys.|
